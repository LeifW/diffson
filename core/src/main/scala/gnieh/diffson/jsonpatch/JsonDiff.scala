...
object JsonDiff {
  ...
  def provider[JsValue](lcsalg: Lcs[JsValue] = new Patience[JsValue], remember: Boolean = false, arrayDiffs: Boolean = true)(implicit J: JsonProvider[JsValue]): DiffProvider[JsValue, JsonPatch] =
    new DiffProvider[JsValue, JsonPatch] {
      val d = new JsonDiff(lcsalg, remember, arrayDiffs)
 ...
 */
class JsonDiff[JsValue](lcsalg: Lcs[JsValue], remember: Boolean, arrayDiffs: Boolean)(implicit J: JsonProvider[JsValue]) {
   ...
   */
  def diff(json1: JsValue, json2: JsValue): JsonPatch[JsValue] =
    new JsonPatch(diff(json1, json2, remember, arrayDiffs = true, Pointer.Root))

  private def diff(json1: JsValue, json2: JsValue, remember: Boolean, arrayDiffs: Boolean, pointer: Pointer): List[Operation[JsValue]] = (json1, json2) match {
  ...
  private def fieldsDiff(fields1: List[(String, JsValue)], fields2: List[(String, JsValue)], remember: Boolean, arraysDiff: Boolean, path: Pointer): List[Operation[JsValue]] = {
    ...
    @tailrec
    def fields(fs: List[(Option[(String, JsValue)], Option[(String, JsValue)])], acc: List[Operation[JsValue]]): List[Operation[JsValue]] = fs match {
  ...
  private def arraysDiff(arr1: List[JsValue], arr2: List[JsValue], remember: Boolean, path: Pointer): List[Operation[JsValue]] = {
    ...
    @tailrec
    def add(arr: List[JsValue], idx: Int, acc: List[Operation[JsValue]]): List[Operation[JsValue]] = arr match {
      case v :: tl => add(tl, idx + 1, Add(path / idx, v) :: acc)
      case Nil     => acc.reverse
    }

    // remove a bunch of array elements starting by the last one in the range
    def remove(from: Int, until: Int, shift: Int, arr: List[JsValue]): List[Operation[JsValue]] =
    ...
    def loop(
      ...
      acc: List[Operation[JsValue]] // the already accumulated result
    ): List[Operation[JsValue]] = (arr1, arr2) match {
